...
4. Commits to the project's current branch as the repository owner, with no attribution trailer
5. Pushes only when the owner asks
...
- **The Git workflow** — the host repo's rules (see its `CLAUDE.md`): commit as the owner, never add attribution, push only on request
...
Claude Code reads all relevant pages, updates all references, commits as the owner.
...
**Git provides the safety net.** Every change is a commit. The owner reviews the commit diffs before merging.
...
- [Style Guide](style-guide.md) — writing conventions for wiki pages
- [Project overview](../overview.md) — example documentation page

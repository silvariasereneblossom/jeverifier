...
|---|---|---|
| `ctx find` | Reading lists of 12–25k tokens against ~168k for all docs. In the first real milestone, the orchestrator opened ~9.5k tokens beyond its 25k list | It ranks prose, so it never sees function signatures: a task that pins an API still needs a code read. It can also skip one sibling of a group it otherwise includes |
